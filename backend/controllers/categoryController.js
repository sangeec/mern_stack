...
const saveAttr = async (req, res, next) => {
    ...
    try {
        ...
        if (categoryExists.attrs.length > 0) {
            // if key exists in the database, then add a value to the database
            ...
            categoryExists.attrs.map((item, idx) => {
                if (item.key === key) {
                    ...
                    var newAttributeValues = [...new Set(copyAttributeValues)] // to ensure unique values
        ...
        let categ = await Category.find({}).sort({name: "asc"})
        return res.status(201).json({categoriesUpdated: categ})
    } catch (error) {
        next(error)
    }
}
